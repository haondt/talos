...
{
    public class ImageUpdaterService(ILogger<ImageUpdaterService> logger,
        IImageUpdateDataRepository updateDataRepository,
        INotificationService notificationService,
        IImageParser imageParser,
        ISkopeoService skopeoService,
        IOptions<ImageUpdaterSettings> options,
        ITracer<ImageUpdaterService> tracer) : IImageUpdaterService

    {
        ...
        {
            using (logger.BeginScope(FlattenObject(trace)))
                logger.LogInformation("Processed image update {CurrentImage}", trace.Identity);
        }
        public async Task<Optional<ScheduledPushWithIdentity>> HandleImageUpdateAsync(UpdateIdentity id, IUpdateLocation location)
        {
            using var span = tracer.StartSpan($"{nameof(HandleImageUpdateAsync)}");
                ...
                case BumpStrategy.Prompt:
                    {
                        if (cached.IsSuccessful)
                            if (cached.Value.Interaction.HasValue)
                            {
                                ...
                                {
                                    await notificationService.DeleteInteraction(cached.Value.Interaction.Value.InteractionId.Value!);
                                    cached.Value.Interaction.Value.InteractionId = new();
                                }
                            }
                        var interactionId = await notificationService.CreateInteractionAsync(idPush);
                ...
                case BumpStrategy.Notify:
                    {
                        ...
                        await notificationService.Notify(idPush);
        ...
        {
            var inspect = await skopeoService.Inspect($"{image.Untagged}:{tag}");
        ...
        public async Task<List<ParsedTag>> GetSortedCandidateTagsAsync(ParsedImage parsedActiveImage, BumpSize maxBumpSize)
        {
            ...
            var tags = await skopeoService.ListTags(parsedActiveImage.Untagged);
        ...
        public async Task CompletePushAsync(ScheduledPushWithIdentity push, IUpdateLocationSnapshot snapshot)
        {
            ...
            if (cached.Interaction.HasValue)
                if (!cached.Interaction.Value.PendingPush.IsNewerThan(push.Push))
                {
                    if (cached.Interaction.Value.InteractionId.HasValue)
                        await notificationService.DeleteInteraction(cached.Interaction.Value.InteractionId.Value);
